...
def count_code(s):
    ...
    for num in range(len(s)-3):
        if s[num:num+2] == 'co' and s[num+3] == 'e':
            count += 1
    return count
...
def end_other(a, b):
    # print(a[-(len(b)):])
    return b.lower() == a[-(len(b)):].lower() or a.lower() == b[-(len(a)):].lower()
...
def soma_na_lista(n, lista):
    for pos in range(len(lista)):
        for other in range(len(lista)-pos):
            if lista[pos] != lista[other+pos] and lista[pos] + lista[other+pos] == n:
                return True
    return False
