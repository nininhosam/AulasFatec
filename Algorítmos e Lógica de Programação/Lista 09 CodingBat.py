...
def middle_way(a, b):
    return [a[(len(a)-int(len(a)/2))-1], b[(len(b)-int(len(b)/2))-1]]
...
def squirrel_play(temp, is_summer):
    return (60 <= temp <= 90 and not is_summer) or (60 <= temp <= 100 and is_summer)
...
def alarm_clock(day, vacation):
    if day in [0, 6] and vacation:
        return 'off'
    if day in [0, 6] and not vacation:
        return '10:00'
    if vacation:
        return '10:00'
    return '7:00'
